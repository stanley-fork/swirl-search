...
| Status | Meaning | 
| ---------- | ---------- |
| **ERR_DUPLICATE_RESULT_OBJECTS** | More than one Result object was found; [contact support](mailto:hello@swirlaiconnect.com) for assistance. |
...
| **ERR_RESULT_NOT_FOUND** | A Result object that was expected to be found was not; [contact support](mailto:hello@swirlaiconnect.com) for assistance. |
...
| **date_published** | Source-reported publication date (if available, otherwise `unknown`). Used by the [DateMixer](#mixers). | `unknown` |
...
{: .highlight }
Feedback and suggestions on improving this setup are [welcome](mailto:hello@swirlaiconnect.com)!
...
Note that the example above only applies to calling the SWIRL API. Please [contact support](mailto:hello@swirlaiconnect.com) for instructions on how to configure this behavior with the Galaxy UI. 
